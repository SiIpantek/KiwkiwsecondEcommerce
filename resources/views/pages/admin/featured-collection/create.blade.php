...
@section('title', 'Tambah Koleksi Terbaru | E-Commerce')

@section('content')
<div class="max-w-screen-xl px-4 py-8 mx-auto sm:px-6 lg:px-8">
  <header class="mb-8">
    <h1 class="text-3xl font-bold text-gray-900">Tambah Koleksi Terbaru</h1>
    <p class="mt-2 text-gray-600">Tambahkan foto koleksi terbaru yang akan ditampilkan di halaman produk.</p>
  </header>

  <form action="{{ route('dashboard.featured-collections.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
    @csrf
        ...
        <div>
          ...
          <div class="flex flex-col md:flex-row gap-6">
            <div class="flex-shrink-0">
              <div class="w-full md:w-64 h-48 border-2 border-gray-300 rounded-lg overflow-hidden bg-gray-50 flex items-center justify-center" id="image-preview-container">
                <span class="text-gray-400 text-sm" id="image-placeholder">Preview gambar</span>
                <img src="" alt="Preview" id="image-preview" class="w-full h-full object-cover" style="display: none;">
              </div>
            </div>
            <div class="flex-1">
              <input 
                ...
                onchange="previewImage(this, 'image-preview', 'image-placeholder')"
              >
              @error('image')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
              @enderror
